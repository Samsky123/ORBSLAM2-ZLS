    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    //opencv提供了文件的读写操作，从配置文件中找到对应的字符串，可以将数据字符转成float类型
    ...
    //根据cv中Mat的格式创建一个内参矩阵，之后可以用K矩阵直接进行计算
    //     |fx  0   cx|
    // K = |0   fy  cy|
    //     |0   0   1 |
    ...
    // 双目摄像头baseline * fx 50;mbf是tracking这个类的成员变量，成员函数tracking是可以随便地使用成员变量的。
    ...
    //定义一个特征提取器对象，需要的参数有：特征点数、金字塔变化尺度、金字塔层数、特征点默认阈值、特征点最小阈值
    ...
    //定义一个初始化时候特征提取器的对象：和上一个不同的是提取的特征点数是普通提取器的2倍这样可以提高初始化的成功概率
    ...
    {
        // 判断一个3D点远/近的阈值 mbf * 35 / fx；和基线长度有关，基于基线的测量有一个最远的距离，根据这个阈值来判断远近
...
//mpLocalMapper是一个LocalMapping的一个类，可以完成局部建图的功能
//实际上就是建立了一个新的线程
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
    ...
    // 步骤2：构造Frame
    //Frame是一个类，由普通的图像cv::Mat转化为Frame需要加入时间戳、特征提取器、orb词库、相机参数、畸变参数、深度阈值、基线等
    //单目、双目、RGBD分别有三个不同的Frame构造方法
...
void Tracking::Track()
{
    ...
    else// 步骤2：跟踪
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            {
                // BOW搜索，PnP求解位姿
		        //关键帧和运动模型都失效了，此时利用全局重定位来初始化位姿估计
                bOK = Relocalization();
            }
        }
        else //模式：只有定位模式
        ...
        {
            // mbVO true means that there are few matches to MapPoints in the map. We cannot retrieve（重新得到)
